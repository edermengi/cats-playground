...
object TypeClasses {
  ...
  implicit object PersonSerializer extends JSONSerializer[Person] {
    ...
    override def toName(value: Person): String = value.name
  }
  ...
  object JSONSyntax {
    implicit class JSONSerializable[T](value: T)(implicit serializer: JSONSerializer[T]) {
      ...
      def toName: String = serializer.toName(value)
    }
  }

  def main(args: Array[String]): Unit = {
    println(convertListToJSON(List(Person("Alice", 123), Person("John", 33))))

    import JSONSyntax._
    val bob = Person("Bobsl", 35)
    println(bob.toJson)
    println(bob.toName)
  }
}
